...
if ! git remote | grep -q "^slate_blog$"; then
    git remote add slate_blog "$SLATE_BLOG_REPO" || { echo "Failed to add Blog remote repository"; exit 1; }

    echo "Successfully added Blog remote repository"
fi

git fetch --no-tags slate_blog || { echo "Failed to fetch code from Blog"; exit 1; }
...
git remote remove slate_blog || { echo "Failed to remove Blog remote repository"; exit 1; }
